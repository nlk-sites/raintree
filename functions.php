...
define('NECTAR_THEME_NAME', 'raintree');
...
function nectar_register_js() {	
	...
	if (!is_admin()) { 
		...
		wp_register_script('blogmasonry', get_template_directory_uri() . '/js/blog-masonry.js', 'jquery', '3.0.5', TRUE);
		wp_register_script('carouFredSel', get_template_directory_uri() . '/js/carouFredSel.min.js', 'jquery', '6.2', TRUE);
		wp_register_script('nectarSlider', get_template_directory_uri() . '/js/nectar-slider.js', 'jquery', '3.0.5', TRUE);
		wp_register_script('custom-js', get_template_directory_uri() . '/js/custom.js', 'jquery', '1.0', TRUE);

		if ( floatval(get_bloginfo('version')) < "3.6" ) {
			wp_register_script('jplayer', get_template_directory_uri() . '/js/jplayer.min.js', 'jquery', '2.1', TRUE);
		}
		wp_register_script('nectarFrontend', get_template_directory_uri() . '/js/init.js', array('jquery', 'superfish', 'carouFredSel', 'easing', 'flexslider', 'nicescroll'), '3.0.5', TRUE);
...
function nectar_page_specific_js() {
	...
	//home
	if ( is_page_template('template-home-1.php') || $template_name == 'raintree/template-home-1.php' ||
	     is_page_template('template-home-2.php') || $template_name == 'raintree/template-home-2.php' ||
	     is_page_template('template-home-3.php') || $template_name == 'raintree/template-home-3.php' ||
	     is_page_template('template-home-4.php') || $template_name == 'raintree/template-home-4.php') {
		wp_enqueue_script('orbit');
	}
	
	//contact
	if ( is_page_template('template-contact.php') ||  $template_name == 'raintree/template-contact.php') {
...
function nectar_main_styles() {	
		 ...
		 wp_register_style("main-styles", get_stylesheet_directory_uri() . "/style.css", '', '3.0.5');
		 wp_register_style("responsive", get_template_directory_uri() . "/css/responsive.css", '', '3.0.5');
		...
		<div style="height: '.$config_arr['slider_height'].'px" class="swiper-container" data-loop="'.$config_arr['loop'].'" data-height="'. $config_arr["slider_height"] .'" data-arrows="' . $config_arr["arrow_navigation"].'" data-bullets="'.$config_arr["bullet_navigation"].'" data-desktop-swipe="'. $config_arr["desktop_swipe"].'" data-settings="">
			   ...
			   $slider .= '</div>';
			      ...
			      if($config_arr['arrow_navigation'] == 'true' && $slide_count > 1) {
				     $slider .= '<a href="" class="slider-prev"><i class="icon-raintree-left-arrow"></i> <div class="slide-count"> <span class="slide-current">1</span> <i class="icon-raintree-right-line"></i> <span class="slide-total"></span> </div> </a>
			     		<a href="" class="slider-next"><i class="icon-raintree-right-arrow"></i> <div class="slide-count"> <span class="slide-current">1</span> <i class="icon-raintree-right-line"></i> <span class="slide-total"></span> </div> </a>';
			       } 
...
add_action('woocommerce_before_main_content', 'raintree_theme_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'raintree_theme_wrapper_end', 10);

function raintree_theme_wrapper_start() {
    echo '<div class="container main-content">';
}

function raintree_theme_wrapper_end() {
    echo '</div>';
}
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	ob_start(); ?>
	<a class="cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>"><div class="cart-icon-wrap"><i class="icon-raintree-cart"></i> <div class="cart-wrap"><span><?php echo $woocommerce->cart->cart_contents_count; ?> </span></div> </div></a>
	<?php
...
//[the_template_url]
function template_url_shortcode( $atts ){
	return get_template_directory_uri();
}
add_shortcode( 'the_template_url', 'template_url_shortcode' );
